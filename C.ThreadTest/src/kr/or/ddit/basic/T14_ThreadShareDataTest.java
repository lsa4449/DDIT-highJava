 ...
 * 4. 각각의 쓰레드는 이 인스터느의 참조값을 저장한 변수를 이용하여 공통 데이터를 사용한다.
 * 
 * 예) 원주율을 계산하는 쓰레드가 있고, 계산된 원주율을 출력하는 쓰레드가 있다. 원주율을 계산한 후 이 값을 출력하는 프로그램을 작성하시오.
...
class ShareData{
   public double result; // 원주율이 저장 될 변수
   /*
   ...
   public void run() {
       ...
       */
		...
		for (int i = 1; i <= 1500000000; i += 2) {
			if (((i / 2) % 2) == 0) { // 2로 나눈 몫이 짝수이면(2로 나누었을 때 나머지0)
				sum += (1.0 / i);
			} else {
				sum -= (1.0 / i);
			}
