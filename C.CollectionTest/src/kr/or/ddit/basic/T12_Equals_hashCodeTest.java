...
public class T12_Equals_hashCodeTest {
	 ...
	 *  호출한다. 그래서, 객체가 서로 같은지 여부를 결정하려면 두 메서드를 재정의
	 *  해야 한다. HashSet, HashMap, Hashtable에서는 객체가 같은지 여부는
	 *  데이터를 추가할 때 검사한다.
	 * 
	 * - equals()메서드는 두 객체의 내용(값)이 같은지를 비교하는 메서드 이고,
	 * - hashCode()매서드는 두 객체가 같은 객체인지를 비교하는 메서드 이다.
	 * 
	 * - equals()메서드와 hashCode()메서드에 관련된 규칙
	 *  1. 두 객체가 같으면 반드시 같은 hachCode를 가져야 한다.
	 ...
	 *     기반으로 한 정수 값을 반환한다.
	 *     그러므로, 클래스에서 hashCode()메서드를 override하지 않으면 절대로 두 객체가 
	 *     같은 것 으로 간주될 수 없다.
	 *     
	...
	public static void main(String[] args) {
		...
		System.out.println("rempve(p2) 후 데이터 ...");
		for(Person p : set) {
			System.out.println(p1.getId() + " : " + p.getName());
